// Copied from /static/itinerary.js
document.addEventListener('DOMContentLoaded', () => {
    ...
    saveItineraryBtn.onclick = async () => {
        if (!localStorage.getItem('token')) { alert('Please login to save your itinerary'); return; }
        try {
            ...
            if (response.ok) { alert('Itinerary saved successfully!'); }
            else { alert('Failed to save itinerary'); }
        } catch (error) { alert('An error occurred while saving'); }
    };

    downloadItineraryBtn.onclick = () => { alert('PDF download feature coming soon!'); };
});
